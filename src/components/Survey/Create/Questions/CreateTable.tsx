...
export function CreateTable({ question, updateProp, syncSurvey }: QuestionProps) {
  ...
  const addOption = () => {
    ...
    const updatedQuestion = {
      ...question,
      options: [...options, { label: `Option ${question.options?.length!! + 1}`, value: `Option ${question.options?.length!! + 1}` }]
    };
    updateProp(updatedQuestion);
    syncSurvey(updatedQuestion);
  };
  ...
  const removeOption = (index: number) => {
    ...
    const updatedQuestion = {
      ...question,
      options: question.options.filter((op, i) => i !== index)
    };
    updateProp(updatedQuestion);
    syncSurvey(updatedQuestion);
  };
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      ...
      <div className="flex gap-2">
        <Button
          className="flex items-center px-3 py-2 justify-center gap-2 bg-zinc-50 shadow-md hover:bg-zinc-100 rounded-2xl"
          variant="tertiary"
          onClick={addRow}
        >
          <IoMdAdd /> Linha
